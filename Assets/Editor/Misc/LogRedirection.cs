...
static class LogRedirection
{
    ...
    private static readonly Regex s_luaLogRegex = new Regex(@"([a-zA-Z0-9|\.]+)\:(\d+)\: (unexpected|in) (.+)\r?\n");
    ...
    private static bool OnOpenAsset(int instanceId, int line)
    {
        ...
        Match match = s_luaLogRegex.Match(selectedStackTrace);
        if (!match.Success)
        {
            return false;
        }

        string gcmd = "  " + Application.dataPath + "/../lua" + "/" + match.Groups[1].Value.Replace(".","/") + ".lua:" + match.Groups[2].Value;
        RunCmd("subl", gcmd);
        return true;
    }
